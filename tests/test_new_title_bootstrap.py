...
def test_new_title_bootstraps_valid_title_scoped_project_without_lint(tmp_path: Path) -> None:
    ...
    assert (project / 'docs/automation/director_profile.json').exists()
    assert (project / 'docs/automation/auto_approval_policy.json').exists()
    assert (project / 'docs/automation/vision_scorecard.schema.json').exists()
